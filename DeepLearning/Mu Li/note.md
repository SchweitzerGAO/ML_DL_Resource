...
The circled arrows means concatenation at `dim=1`

3. When training, the input of the decoder is ground truth:
...
#### b. attention score function

To visualize how attention works:

![](./image/202.PNG)

The score function in part a is $a(x) =\text{softmax}(-\dfrac{1}{2}(x-x_i)^2 )$

**additive attention**

When queries and keys are not homodimensional vectors, additive attention can be applied, the score function is:

$$
a(\mathbf k,\mathbf q) = \mathbf v^T\tanh(\mathbf W_k\mathbf k+\mathbf W_q\mathbf q)
$$

where:

$\mathbf k \in \mathbb R^{k}$ is key and $\mathbf q \in \mathbb R^{q}$ is query $\mathbf v \in \mathbb R^{v}$ is value,

$\mathbf W_k \in \mathbb R^{v\times k}$, $\mathbf W_q \in \mathbb R^{v\times q}$ are learnable parameters
...
$$
a(\mathbf q, \mathbf k_i) = \frac{\mathbf q \cdot \mathbf k_i}{\sqrt{d} }
$$

where: 

$\mathbf q, \mathbf k_i \in \mathbb R^d$

vectorized version:

$$
a(\mathbf Q, \mathbf K) = \frac{\mathbf Q \mathbf K^T}{\sqrt{d} } 
$$
...
$$
f = \text{softmax}(a(\mathbf Q, \mathbf K) ) \mathbf V
$$
...
When translating 'hello' to 'bonjour', the context should **only** be the encoder-RNN output of 'hello'. That's why attention is applied.

**structure of model**

![](./image/204.png)

*key, value: the output (hidden state)of encoder-RNN*

*query: the output (hidden state) of decoder-RNN*
...
Alike inception blocks in CNN, more info can be extracted from the training data.

the parameters and output of multihead attention are:
...
- Position-wise Foward Feeding Network(FFN,i.e. MLP)
...
- Transformer encoder = encoder block + positional encoding
...
- Transformer decoder = decoder block + positional encoding
